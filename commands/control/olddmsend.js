const { Command } = require('discord.js-commando');

module.exports = class olddmsend extends Command {
	constructor(client) {
		super(client, {
			name: 'olddmsend',
			group: 'control',
			memberName: 'olddmsend',
			description: 'make the bot send somthing',
			ownerOnly: true,
		});
	}
 
    run(message, client) {
		const fs = require('fs')
        let rawsend = fs.readFileSync('./dmchannel.json');
        let send = JSON.parse(rawsend);
		let rawsay = fs.readFileSync('./dmwillsay.json');
		let cansay = JSON.parse(rawsay);
		this.client.users.cache.get(send.channel).send(cansay.say)
		return message.say(`message sent to <@${send.channel}> saying: ${cansay.say}`)
	}
};